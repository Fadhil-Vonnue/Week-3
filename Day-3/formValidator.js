const rules = {
  ...
  password: [{ rule: "required" }, { rule: "minLength", minLength: 8 }],
  confirmPassword: [
    { rule: "required" },
    { rule: "match", otherField: "password" },
  ],
  username: [
    { rule: "required" },
    {
  ...
  role:[{ rule: "required" }],
  phone: [{ rule: "required" }, { rule: "pattern", regex: /[0-9]{10}/ }],
  cv: [{ rule: "required" }],
  coverLetter: [{ rule: "required" }],
};
class FormValidator {
    ...
    this.form.addEventListener(
      ...
      (e) => {
        if (e.target.tagName === "INPUT") {
          console.log("Input left:", event.target.name, event.target.value);
    ...
    if (this.rules[field.name]) {
      for (const rule of this.rules[field.name]) {
        switch (rule.rule) {
          ...
          case "minLength":
            if (rule.minLength > field.value.length && field.value) {
          ...
          case "match":
            if (
              document.querySelector(`[name="${rule.otherField}"]`).value ==
              field.value
            ) {
              break;
            } else {
          ...
          case "custom":
            ...
            //   alert("not an admin");
            field.nextElementSibling.textContent=field.nextElementSibling.textContent+"not an admin"
              flag=1
            } else break;
        }
      }
