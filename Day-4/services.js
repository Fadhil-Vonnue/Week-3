...
function debounce() {
  const id = setTimeout(findCards(), 300);
}
...
window.addEventListener("popstate", (event) => {
  if (event.state) {
    ...
    document.getElementById("searchCard").value = event.state.searchquery;
  }
});
function findCards1(){
    findCards()
    ...
    history.pushState(result, "", `service.html?query=${document.getElementById("searchCard").value}`);
  } catch (err) {
    alert(err);
  }
}